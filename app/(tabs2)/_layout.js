  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        />
        <Tabs.Screen
          name={isRtl ? 'diet/dietScreen' : 'workout/workoutScreen'}
          options={{
            tabBarIcon: ({ color }) =>
              isRtl
                ?
                <MaterialCommunityIcons name="pot-steam-outline" size={24} color={color} />
                :
                <MaterialIcons name="fitness-center" size={24} color={color} style={{ transform: [{ rotate: '-40deg' }] }} />,
            tabBarLabel: ({ focused }) => tabBarLabel({ label: isRtl ? tr('diet') : tr('workout'), focused })
          }}
        />
        <Tabs.Screen
          name="insight/insightScreen"
          options={{
            tabBarIcon: ({ color }) => <MaterialIcons name="collections-bookmark" size={24} color={color} />,
            tabBarLabel: ({ focused }) => tabBarLabel({ label: tr('insight'), focused })
          }}
        />
        <Tabs.Screen
          name={isRtl ? 'workout/workoutScreen' : 'diet/dietScreen'}
          options={{
            tabBarIcon: ({ color }) =>
              isRtl
                ?
                ...
                :
                <MaterialCommunityIcons name="pot-steam-outline" size={24} color={color} />,
            tabBarLabel: ({ focused }) => tabBarLabel({ label: isRtl ? tr('workout') : tr('diet'), focused })
          }}
        />
        <Tabs.Screen
